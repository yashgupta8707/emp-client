...
const ItemsTable = () => {
  const { selectedItems, addItemRow, removeItemRow, updateItem } =
    useQuotation();
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-700 text-gray-200">
              ...
              <th className="p-3 text-right">Purchase (w/o GST)</th>
              <th className="p-3 text-right">Purchase Price</th>
              <th className="p-3 text-right">Sale (w/o GST)</th>
              ...
              <tr key={item.id} className="border-b border-gray-700">
                ...
                <td className="p-3">
                  <select
                    ...
                    onChange={(e) =>
                      updateItem(item.id, "component", e.target.value)
                    }
                    className="w-full bg-gray-600 text-white border border-gray-500 rounded px-2 py-1 focus:outline-none focus:border-blue-500"
                  ...
                  >
                    ...
                    {item.category &&
                      componentDatabase[item.category].map((comp) => (
                ...
                <td className="p-3">
                  <input
                    ...
                    value={item.warranty}
                    onChange={(e) =>
                      updateItem(item.id, "warranty", e.target.value)
                    }
                ...
                <td className="p-3 text-right">
                  ₹
                  {item.purchasePrice
                    ? (item.purchasePrice / 1.18).toFixed(2)
                    : "0.00"}
                </td>
                <td className="p-3">
                  <input
                ...
                <td className="p-3 text-right">
                  ₹
                  {(item.isCustomPrice ? item.customPrice : item.basePrice)
                    ? (
                        (item.isCustomPrice
                          ? item.customPrice
                          : item.basePrice) / 1.18
